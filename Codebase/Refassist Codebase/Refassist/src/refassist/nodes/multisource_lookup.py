...
async def multisource_lookup(state: PipelineState) -> PipelineState:
    ...
    for s in sources:
        ...
        if doi: tasks.append(s.by_doi(doi))
        if title: tasks.append(s.by_title(title))
    ...
    for s in sources:
        if arxiv_id and isinstance(s, ArxivClient):
            ...
            if rec: out_norm.append(_normalize_candidate(s.NAME, rec))
        if doi:
            rec = results[idx]; idx+=1
            if isinstance(rec, list):
                for r in rec: out_norm.append(_normalize_candidate(s.NAME, r))
