...
from ..nodes import (
    ...
    format_reference, build_exports, build_report, cleanup, route_after_verify, _route_after_parse
)
...
from ..nodes.llm_format import llm_format  
...
def build_graph(cfg: PipelineConfig = PipelineConfig()) -> StateGraph:
    ...
    g.add_node("LLMFormat", llm_format)             
    ...
    g.add_conditional_edges(
        "ParseExtract",
        _route_after_parse,
        {"VerifyJournalAbbrev": "VerifyJournalAbbrev", "MultiSourceLookup": "MultiSourceLookup"},
    )
...
async def run_one(reference: str, cfg: PipelineConfig = PipelineConfig(), recursion_limit: int | None = None):
    ...
    - Automatically saves a Mermaid PNG of the current pipeline.
    """
    global _COMPILED
    ...
    try:
        png_data = _COMPILED.get_graph().draw_mermaid_png()
        os.makedirs("outputs", exist_ok=True)
        # sanitize reference for filename
        safe_name = "".join(c if c.isalnum() or c in "_-" else "_" for c in reference[:40])
        out_path = os.path.join("outputs", f"pipeline_graph_{safe_name}.png")
        with open(out_path, "wb") as f:
            f.write(png_data)
        print(f"Pipeline graph saved as: {out_path}")
        # Optional: Display inline (Jupyter/IPython)
        display(Image(png_data))
    except Exception as e:
        print(f"Could not save or display pipeline graph automatically: {e}")
